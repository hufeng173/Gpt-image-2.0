...
export default function Home() {
  ...
  async function loadSettings() {
    try {
      ...
      const data = await readApiJson<SettingsResult>(response);
  ...
  async function uploadReferences(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (!data.ok) {
        throw new Error(data.shortReason || data.message || "上传失败");
      }
  ...
  function buildGeneratePayload(override?: {
    prompt?: string;
  ...
  async function generateImages(override?: {
    prompt?: string;
    selectedImageUrl?: string;
    conversation?: ChatMessage[];
  }) {
    ...
    try {
      ...
      if (!response.ok || !data.ok) {
        throw new Error(data.shortReason || data.message || "生成失败");
      }
      ...
      setGeneratedImages((prev) => {
        const oldImages = prev.filter((item) => !nextImages.some((next) => next.id === item.id));
        return [...nextImages, ...oldImages];
      });
  ...
  async function optimizePrompt() {
    ...
    try {
      ...
      const data = await readApiJson<OptimizeResult>(response);

      if (!data.ok) {
        throw new Error(data.shortReason || data.message || "提示词优化失败");
      }
  ...
  async function optimizeSelectedImageAndGenerate() {
    ...
    try {
      ...
      if (!data.ok) {
        throw new Error(data.shortReason || data.message || "单图优化失败");
      }

      const optimizedPrompt = data.optimizedPrompt || prompt;
      ...
      setImageConversations((prev) => ({
        ...
        [selectedImage.id]: [
          ...
          {
            role: "assistant",
            content: data.reply || "已根据选中图片优化，并开始生成新版本。",
            time: nowText(),
          },
        ],
      }));
  ...
  async function downloadImage(image: GeneratedImage, format: "png" | "jpeg") {
    ...
    htmlImage.onerror = () => {
      setError("图片读取失败，可能是图片文件不存在或公网连接中断。");
    };
              ...
              <div className="reference-list">
                {references.map((item) => (
                  <button
                    key={item.url}
                    type="button"
                    onClick={() => setReferences((prev) => prev.filter((x) => x.url !== item.url))}
                  >
            ...
            <div className="context-box">
              ...
              <div className="chat-mini">
                {globalConversation.slice(-4).map((item, index) => (
                  <p key={`${item.time}-${index}`} className={item.role}>
                    {item.content}
                  </p>
                ))}
              </div>
              ...
              <textarea
                value={contextDraft}
                placeholder="例如：更像宋代山水，画面更留白，品牌感更强..."
              ...
              <div>
                ...
                <p>
                  {result?.generatedCount
                    ? `已生成 ${result.generatedCount}/${result.requestedCount} 张`
                    : "点击图片可选中，并进行单图对话优化。"}
                </p>
              </div>
            </div>
            ...
            <section className="selected-dialogue">
              ...
              {selectedImage ? (
                <>
                  ...
                  <textarea
                    value={imageDraft}
                    placeholder="例如：保留主体，把背景改成水墨山水，增加竹影和留白..."
                  ...
                  <button
                    ...
                    type="button"
                    disabled={optimizing || loading || !imageDraft.trim()}
                    onClick={optimizeSelectedImageAndGenerate}
                  >
          ...
          <section className="settings-panel" onClick={(event) => event.stopPropagation()}>
            ...
            <label>
              ...
              <select
                ...
                onChange={(e) => void saveSettings({ defaultImageModel: e.target.value })}
              >
                {settings.imageModels.map((item) => (
                  <option key={item.id} value={item.id}>
                    {item.name}（{item.id}）
            ...
            <label>
              ...
              <input
                ...
                onChange={(e) =>
                  setSettings((prev) => ({
                    ...prev,
                    promptOptimizerModel: e.target.value,
                  }))
                }
                onBlur={(e) => void saveSettings({ promptOptimizerModel: e.target.value })}
              />
            </label>

            <label>
              <span>最大并发生成数（建议 6-8，最高 12）</span>
              <input
                type="number"
                min={1}
                max={12}
                ...
                onChange={(e) =>
                  void saveSettings({
                    maxConcurrentGenerations: Number(e.target.value),
                  })
                }
              />
            </label>
            ...
            <label className="check-line">
              <input
                type="checkbox"
                checked={settings.allowReferenceImageEdit}
                onChange={(e) =>
                  void saveSettings({
                    allowReferenceImageEdit: e.target.checked,
                  })
                }
              />
              优先尝试使用参考图编辑接口，失败后自动转文字生成
            </label>
          </section>
        </div>
      ) : null}
